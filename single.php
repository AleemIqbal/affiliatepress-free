<?php get_header();
affiliatepress_wpb_set_post_views(get_the_ID()); ?>

<div class="columns">
    <div class="column is-9 hfeed">
        <?php while (have_posts()) : the_post();
            $author_img = affiliatepress_get_my_avatar_url(get_the_author_meta("ID"));
            $featured_img = wp_get_attachment_image_src(get_post_thumbnail_id(), "featured");
            $image_url = !empty($featured_image) ? $featured_image[0] : '';
            $image_url1 = !empty($featured_image) ? $featured_image[1] : '';
            $image_url2 = !empty($featured_image) ? $featured_image[2] : '';

            $visibility_title = affiliatepress_get_post_meta_default(get_the_ID(), "_visibility_title");
            $visibility_title = $visibility_title === "default" ? (get_theme_mod("setting_visibility_title", true) ? "show" : "hide") : $visibility_title;

            $visibility_featured_image = affiliatepress_get_post_meta_default(get_the_ID(), "_visibility_featured_image");
            ...
            $visibility_comments = affiliatepress_get_post_meta_default(get_the_ID(), "_visibility_comments");
            ...
            <article class="hentry">
                ...
                <div class="post-meta">
                    ...
                    <time class="updated"><?php affiliatepress_the_svg('calender') ?>Updated <?php echo human_time_diff(affiliatepress_get_the_updated_time("U")) ?> ago</time>
                    <time class="published"><?php the_date() ?></time>
                    <?php if ($visibility_comments != "hide") { ?>
                        <span class="separator">|</span>
                        <span class="comments"><?php affiliatepress_the_svg('comments') ?><a class="comments" href="#disqus_thread">Comments</a></span>
                    <?php } ?>
                </div>
                <?php
                if ($visibility_featured_image != "hide") { ?>
                    <picture>
                        ...
                        <img src="<?php echo esc_url($image_url); ?>" alt="<?php echo get_the_title() ?>" width="<?php echo esc_url($image_url1); ?>" height="<?php echo esc_url($image_url2); ?>" />
                    </picture>
                <?php } ?>
