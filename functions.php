...
function affiliatepress_get_seo_title() {
    ...
    if (is_home()) {
        ...
        $seo_title = ("You searched for " . get_search_query() . " – " . get_bloginfo("name"));
    } else {
...
function affiliatepress_custom_additional_styles()
{
    ...
    if (is_single()) {
        $post_type = "single";
        echo '<link rel="stylesheet" href="/wp-includes/css/dist/block-library/style.min.css' . '" type="text/css" media="print" onload="this.media=`all`" />';
    } elseif (is_category()) {
        $post_type = "category";
    } elseif (is_author()) {
        $post_type = "author";
...
function affiliatepress_customizer_settings($wp_customize)
{
    ...
    $wp_customize->add_setting('setting_copyright_text', array(
        'default' => sprintf('%%site_title%% © %%current_year%%. All rights reserved. Designed By <a href="%1$s" target="_blank">%2$s</a>.', get_theme_mod('theme_author_uri'), get_theme_mod('theme_author')),
        'sanitize_callback' => 'sanitize_text_field'
    ));
    ...
    $wp_customize->add_control("control_twitter_handle", array(
        'label' => __('Twitter Handle', 'affiliatepress'),
...
function affiliatepress_my_register_sidebars()
{
    register_sidebar(array(
        'id'            => 'primary',
        'name'          => __('Post Sidebar', 'affiliatepress'),
        'description'   => __('A short description of the sidebar.', 'affiliatepress'),
